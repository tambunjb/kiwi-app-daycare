...
reportPdf(context, data, milks, naps, setData, formSubmit, isToday) async {
  try {
    ...
    final String path = '$dir/' +
        data['child_name'].toString().replaceAll(RegExp(r"\s+"), "") + '_' +
        DateFormat('ddMMyyyy')
            .format(DateTime.parse(data['report']['date'].toString().split('.')[0]))
            .toString() + '_report.pdf';
    final File file = File(path);
    await file.writeAsBytes(List.from(await pdf.save()));

    await Share.shareFilesWithResult([path]).then((result) async {
      if (result.status == ShareResultStatus.success) {
        setData('shared_at', DateTime.now().toString().split('.')[0]);
        formSubmit(isBack: false, sharePastDate: !isToday);
      }
    });
  } catch(e, stacktrace) {
    final DeviceInfoPlugin deviceInfoPlugin = DeviceInfoPlugin();
    Map desc = _readAndroidBuildData(await deviceInfoPlugin.androidInfo);
    log('LOG - reportPdf - Catch => '+(await Api.addConfig(name: 'log_kid_reportpdf_'+(desc['model'].toString()+desc['board'].toString()).replaceAll(RegExp(r"\s+"), "")+'_'+DateTime.now().millisecondsSinceEpoch.toString(), value: e.toString()+"\n"+stacktrace.toString(), desc: desc.toString())).toString());
  }
}
